...
function plugin(analytics) {
    ...
    analytics.compare = function (A, B) {
        ...
        _.each(bGlobals, function (key) {
            assert(
                _.includes(aGlobals, key),
    ...
    analytics.loaded = function (integration, str) {
        ...
        assert(
            _.includes(this.spies, integration.load),
        ...
        _.each(integration.load.returns, function (el) {
            var tag = {};
    ...
    /**
     * Remove unwanted script/iframe attrs
     *
    ...
    function stripAttrs(tagType, attrs) {
        var ret = _.extend({}, attrs);
        ...
        switch (tagType) {
            ...
            case 'iframe':
                ...
                delete ret.height;
                delete ret.style;
                delete ret.scrolling;
                delete ret.frameBorder;
                break;
            default:
            // no default case
        }
     ...
     * @return {Tester}
     */
    _.each(_.keys(assert), function (key) {
